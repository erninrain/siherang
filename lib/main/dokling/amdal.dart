...
class amdalPage extends StatefulWidget{
  String ur;
  String tit;
  amdalPage({this.tit, this.ur});
  @override
  _amdalPage createState() => _amdalPage();
}
class _amdalPage extends State<amdalPage>{
  ...
  Future<void> kaon() async {
    ...
    final response = await http.get(linknya.urlbase + "app/dokling?nama=AMDAL" );
  ...
  Future<Null> notifikasi()async{
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    String userId = prefs.getString('id');
    await http.get(linknya.urlbase + "app/clearnotif?userId="+ userId +"&menu=1" );
  }
  ...
  _tabBar(){
    return TabBar(
      ...
      tabs: <Widget>[
        Tab ( child: AutoSizeText('Pengertian AMDAL',style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),), ),
        Tab ( child: AutoSizeText('Pengajuan AMDAL',style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),), ),
      ],
    );
  }
