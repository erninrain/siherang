...
class _SpplPage extends State<SpplPage>{
  ...
  Future<Null> notifikasi()async{
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    String userId = prefs.getString('id');
    await http.get(linknya.urlbase + "app/clearnotif?userId="+ userId +"&menu=3" );
  }
  ...
  _tabBar(){
    return TabBar(
      ...
      tabs: <Widget>[
        Tab ( child: AutoSizeText('Pengertian SPPL',style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),), ),
        Tab ( child: AutoSizeText('Pengajuan SPPL',style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),), ),
      ],
    );
  }
